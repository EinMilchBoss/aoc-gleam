...
pub fn parse(input: String) -> List(EnginePart) {
  ...
  |> list.index_map(fn(line, y) {
    parse_line(string.to_graphemes(line), position.Position(0, y), 0, 0, [])
  })
  |> list.flatten()
}
...
fn parse_line(
  ...
  case graphemes {
    [] if part_length > 0 -> [
      EnginePart(part_number, position.y, position.x - part_length, part_length),
      ..acc
    ]
    [] -> acc
    [current, ..rest] -> {
...
pub fn positions(engine_part: EnginePart) -> List(Position) {
  iterator.range(engine_part.start, engine_part.start + engine_part.length - 1)
